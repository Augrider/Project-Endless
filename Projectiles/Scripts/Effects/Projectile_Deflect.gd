...
func _on_projectile_hit(projectile: Projectile) -> void:
	...
	projectile.rotation_degrees = get_projectile_direction(projectile)
	#And add some velocity and power
	projectile.add_power(owner_projectile.power)
	#And reduce our own
	owner_projectile.reduce_power(1)

func get_projectile_direction(projectile:Projectile):
	var target_rotation:float = owner_projectile.rotation_degrees
	var change_direction: int = 1 if randi() % 1 > 0 else -1
	target_rotation += randf_range(-5, 5)
	
	if projectile.power > owner_projectile.power:
		target_rotation += change_direction * 150 * (projectile.power-owner_projectile.power)/10
