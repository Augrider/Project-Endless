...
@export var max_spread: float = 30
...
func perform(arena: Arena):
	...
	for i in shot_count:
		...
		var spread: float = _calculate_spread(i)
...
func _calculate_spread(index: int) -> float:
	return max_spread * (index / (shot_count - 1) - 0.5) + randf_range(-spread_variance, spread_variance)
